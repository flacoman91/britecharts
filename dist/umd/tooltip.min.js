...
!(__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
    ...
    var _require3 = __webpack_require__(287),
        ...
        wrapTextWithEllipses = _require3.wrapTextWithEllipses;
    ...
    return function module() {
        ...
        function drawTooltip() {
            ...
            tooltip = tooltipTextContainer.append('rect').classed('tooltip-text-container', true).attr('x', -tooltipWidth / 4 + tooltipTextLinePadding).attr('y', 0).attr('width', tooltipWidth).attr('height', tooltipHeight).attr('rx', tooltipBorderRadius).attr('ry', tooltipBorderRadius).style('fill', bodyFillColor).style('stroke', borderStrokeColor).style('stroke-width', 1);

            tooltipTextContainer.append('rect').classed('tooltip-title-container', true).attr('x', -tooltipWidth / 4 + tooltipTextLinePadding).attr('y', 0).attr('width', tooltipWidth).attr('height', tooltipHeight + 12).attr('rx', tooltipBorderRadius).attr('ry', tooltipBorderRadius).style('fill', tooltipTitleColor).style('stroke', tooltipTitleColor).style('stroke-width', 1);

            tooltipTitle = tooltipTextContainer.append('text').classed('tooltip-title', true).attr('x', 0).attr('dy', '.35em').attr('y', 12).attr('font-size', '12px').attr('font-weight', '600').style('fill', titleFillColor);

            tooltipDivider = tooltipTextContainer.append('line').classed('tooltip-divider', true).attr('x1', -tooltipWidth / 4 + 10).attr('x2', 265).attr('y1', 25).attr('y2', 25).style('stroke', borderStrokeColor);

            tooltipSubTitle = tooltipTextContainer.append('text').classed('tooltip-subtitle', true).attr('x', 0).attr('dy', '.35em').attr('y', ttTextY + 5).attr('font-size', '18px').attr('font-weight', '500').style('fill', titleFillColor);
        ...
        function updateTopicContent(topic) {
            ...
            elementText = tooltipBody.append('text').classed('tooltip-left-text', true).attr('dy', '1em').attr('x', ttTextX).attr('y', ttTextY).attr('font-size', '12px').attr('font-weight', '500').style('fill', tooltipTextColor).text(tooltipLeftText).call(wrapTextWithEllipses, tooltipMaxTopicLength, initialTooltipTextXPosition, 1);

            tooltipRight = tooltipBody.append('text').classed('tooltip-right-text', true).attr('dy', '1em').attr('x', ttTextX).attr('y', ttTextY).attr('font-size', '12px').attr('font-weight', '500').style('fill', tooltipTextColor).text(tooltipRightText);
            ...
            tooltipBody.append('circle').classed('tooltip-circle', true).attr('cx', 18 - tooltipWidth / 4).attr('cy', ttTextY + circleYOffset).attr('r', 3).style('fill', colorMap[name]).style('stroke-width', '1px');

            tooltipBody.append('line').classed('tooltip-divider', true).attr('x1', -tooltipWidth / 4 + 10).attr('x2', tooltipWidth - (tooltipWidth / 4 - tooltipTextLinePadding)).attr('y1', ttTextY + circleYOffset + 10).attr('y2', ttTextY + circleYOffset + 10).style('stroke', '#43484e');
        ...
        function updatePositionAndSize(dataPoint, xPosition, yPosition) {
            ...
            tooltipDivider.attr('x2', tooltipWidth - (tooltipWidth / 4 - tooltipTextLinePadding));
        }
        ...
        function updateTitle(dataPoint) {
            ...
            var titleWd = tooltipTitle.node().getBoundingClientRect().width;
        ...
        function updateSubTitle(dataPoint) {
            var total = dataPoint.values.reduce(function (acc, cur) {
                return acc + cur.value;
            }, 0);

            var tTitle = subTitle,
                formattedTotal = getFormattedValue(total).toString(),
                subtitleRight = void 0;

            if (tTitle.length) {
                tTitle = '' + tTitle;
            }

            tooltipSubTitle.append('tspan').classed('tooltip-right-title', true).attr('x', -tooltipWidth / 4 + 13).text(tTitle);

            subtitleRight = tooltipSubTitle.append('tspan').classed('tooltip-left-title', true).attr('x', 0).text(formattedTotal);

            var rightST = subtitleRight.node().getBoundingClientRect().width ? subtitleRight.node().getBoundingClientRect().width : subtitleRight;
