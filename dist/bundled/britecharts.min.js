...
!(__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
    ...
    return function module() {
        ...
        getName = function getName(_ref2) {
            ...
            return d3Format.format(labelsNumberFormat)(pctChange) + ' ' + labelsSuffix;
        },
        ...
        _labelsVerticalX = function _labelsVerticalX(_ref9) {
            ...
            _labelsVerticalY1 = function _labelsVerticalY1(_ref11) {
            var pctChange = _ref11.pctChange;
            return yScale2(pctChange) - labelsMargin;
        };
        /**
         * This function creates the graph using the selection as container
         * @param  {D3Selection} _selection A d3 selection that represents
        ...
        function buildAxis() {
            if (isHorizontal) {
                ...
                if (enableYAxisRight) {
                    yAxis2 = d3Axis.axisRight(yScale2).ticks(yTicks, numberFormat).tickFormat(function (d, i) {
                        return d + '%';
                    }).tickSizeOuter(0);
                }
            } else {
        ...
        function buildGradient() {
            if (!chartGradientEl && chartGradientColors) {
                chartGradientEl = svg.select('.metadata-group').append('linearGradient').attr('id', chartGradientId).attr('x1', '0%').attr('y1', '0%').attr('x2', '100%').attr('y2', '100%').attr('gradientUnits', 'userSpaceOnUse').selectAll('stop').data([{ offset: '0%', color: chartGradientColors[0] }, { offset: '50%', color: chartGradientColors[1] }]).enter().append('stop').attr('offset', function (_ref12) {
                    var offset = _ref12.offset;
                    return offset;
                }).attr('stop-color', function (_ref13) {
        ...
        function buildScales() {
            ...
            if (isHorizontal) {
                ...
                yScale2 = d3Scale.scaleBand().domain(data.map(getPctChange)).rangeRound([chartHeight, 0]).padding(betweenRowsPadding);
            } else {
            ...
            colorMap = function colorMap(item) {
                return colorList.filter(function (_ref16) {
        ...
        function drawAxis() {
            ...
            if (isHorizontal && enableYAxisRight) {
                ...
                svg.select('.y-axis-group.axis-right').attr('transform', 'translate(' + chartWidth + ', 0)').call(yAxis2);

                // shift the labels over to the right a bit
                // UNCOMMENT this for the percentages

                svg.selectAll('.y-axis-group.axis-right .tick text').attr('transform', function (d) {
                ...
                svg.selectAll('.y-axis-group.axis-right .tick').append('polygon').attr('transform', function (d) {
        ...
        function drawHorizontalRows(rows, bg) {
            if (bg) {
                // Enter + Update
                rows.enter().append('rect').classed('row', true).attr('y', chartHeight).attr('x', 0).attr('height', yScale.bandwidth()).attr('width', function (_ref17) {
                    var value = _ref17.value;
                    return xScale(value);
                }).on('click', function (d) {
                    handleClick(this, d, chartWidth, chartHeight);
                }).merge(rows).attr('x', 0).attr('y', function (_ref18) {
                    var name = _ref18.name;
                    return yScale(name);
                }).attr('height', yScale.bandwidth()).attr('width', function (_ref19) {
                    var value = _ref19.value;
                    return xScale(value);
                }).attr('fill', function (_ref20) {
                    var name = _ref20.name;
                    return computeColor(name);
                });
            } else {
                // Enter + Update
                rows.enter().append('rect').classed('row', true).attr('y', chartHeight).attr('x', 0).attr('height', yScale.bandwidth()).attr('width', function (_ref21) {
                    var value = _ref21.value;
                    return xScale(value);
                    ...
                    handleClick(this, d, chartWidth, chartHeight);
                }).merge(rows).attr('x', 0).attr('y', function (_ref22) {
                    var name = _ref22.name;
                    return yScale(name);
                }).attr('height', yScale.bandwidth()).attr('width', function (_ref23) {
                    var value = _ref23.value;
                    return xScale(value);
        ...
        function drawAnimatedHorizontalRows(rows, bg) {
            if (bg) {} else {
                // Enter + Update
                rows.enter().append('rect').classed('row', true).attr('x', 0).attr('y', chartHeight).attr('height', yScale.bandwidth()).attr('width', function (_ref25) {
            ...
            if (bg) {
                rows.attr('x', 0).attr('y', function (_ref26) {
                    var name = _ref26.name;
                    return yScale(name);
                }).attr('height', yScale.bandwidth()).attr('fill', backgroundColor).transition().duration(animationDuration).delay(interRowDelay).ease(ease).attr('width', backgroundWidth + '%');
            } else {
        ...
        function drawLabels() {
            var labelXPosition = isHorizontal ? _labelsHorizontalX : _labelsVerticalX;
            var labelYPosition = isHorizontal ? _labelsHorizontalY : _labelsVerticalY;
            var labelYPosition1 = isHorizontal ? _labelsHorizontalY : _labelsVerticalY1;
