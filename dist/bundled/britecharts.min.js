...
!(__WEBPACK_AMD_DEFINE_RESULT__ = function (require) {
    ...
    return function module() {
        ...
        function drawLabels() {
            ...
            // add another group
            if (enableYAxisRight) {
                labelEl2 = svg.select('.metadata-group').append('g').attr('transform', 'translate(' + chartWidth + ', 0)').classed('change-label-group', true).selectAll('g').data(data.reverse()).enter().append('g');

                // each group should contain the labels and rows
                labelEl2.append('text').attr('y', labelYPosition).text(pctChangeText);

                labelEl2.append('polygon').attr('transform', function (d) {
                    var yPos = yScale(d.name) + 20;
